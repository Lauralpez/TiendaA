...
switch($_GET["accion"]) 
{
	case "agregar":
		...
		{
			$codproducto = $usar_db->vaiQuery("SELECT * FROM reclamos WHERE cod='" . $_GET["cod"] . "'");
			$items_array = array($codproducto[0]["cod"]=>array(
			'vai_nombre'	=>$codproducto[0]["nombre"], 
			'vai_cod'		=>$codproducto[0]["cod"],
            'vai_genero'	=>$codproducto[0]["genero"],
            'vai_tipo'		=>$codproducto[0]["tipo"],
	...
	break;	
	case "pagar":
	echo "<script> alert('Gracias por su compra - TiendaAnimal');window.location= 'index.php' </script>";
		unset($_SESSION["items_carrito"]);
...
<meta charset="UTF-8">
<head>
<title>TiendaAnimal</title>
<link href="style.css" rel="stylesheet" />
</head>
<body>
<div align="center"><h1>TiendaAnimal - Carrito de compras</h1></div>
<div>
...
<?php		
    foreach ($_SESSION["items_carrito"] as $item){
		...
		?>
				<tr>
				<td><img src="<?php echo $item["vai_img"]; ?>" class="mesa1" /><?php echo $item["mesa1"]; ?></td>
...
<div class="contenedor_general">
	<?php
	$productos_array = $usar_db->vaiquery("SELECT * FROM reclamos ORDER BY id ASC");
